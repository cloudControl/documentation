...
- `--ssh-key-path` specifies the path of your ssh public key used for the authentication. If the flag is not set, it defaults to `HOME_DIR/.ssh/id_rsa.pub`. The application will try to upload the public key to the platform. See more for key format [Keys](#keys).
