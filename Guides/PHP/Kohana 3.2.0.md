...
We need to do this because Kohana, by default, stores its session files on the filesystem. However, this approach isn't recommended on the dotCloud platform.
...
What that the code's completed for replacing the original environment configuration with one that is based on looking at the setting contained in the dotCloud credentials file setting, *APPLICATION_ENV*, that we set earlier.
...
What that does is to tell Kohana that the cache will be using APC as the backend, which dotCloud provides out of the box and sets the default expiry period to be **3600 seconds**, or **60 minutes**.
...
Now create a file called ``site.php`` under ``application/views/``. In it, add the following code:
    ...
    <html>
        ...
        <body>
            <h1>Welcome to Kohana on dotCloud</h1>
