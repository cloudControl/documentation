...
Visit the new website at `http://development-APP_NAME.cloudcontrolled.com/check/`. Check the list to test if all is working as it should.
...
On the production branch you can check the environment change by watching the cloudControl log and sending a log message with `http://APP_NAME.cloudcontrolled.com/check/`. The cloudControl log should contain only the error messages.
